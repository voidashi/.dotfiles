...
- **fastfetch colours must be written as hex, never as ANSI codes.** fastfetch accepts both, and `38;2;180;73;85` is the same colour as `#b44955` and is invisible to `check_palette.py`. There are five configs under `.config/fastfetch/`: `config.jsonc` is what bare `fastfetch` reads, `arch_small/` is the fish greeter, and `arch_large/`, `crow/` and `home/` are dormant presets kept aligned so they do not come back wrong. Also set `display.percent.type` to `["num"]`: fastfetch otherwise tints a percentage green, yellow or red by threshold, which puts an accent on screen at rest for nothing.
