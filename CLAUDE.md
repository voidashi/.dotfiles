...
- `scripts/unlink-dotfiles.sh`: the inverse of `backup-configs.sh`, removing the symlinks and moving files back to `$HOME`, leaving the repo essentially empty. Interactive confirmation required.
...
- **Qt/KDE theming goes through `.config/kdeglobals`, and `QT_QPA_PLATFORMTHEME` must be `kde`.** Most Qt applications here are KDE ones (dolphin, ark, gwenview, kate, spectacle, partitionmanager, filelight) and read their palette from kdeglobals; the ones that are not, VLC among them, get the same palette through `KDEPlasmaPlatformTheme6.so` from `plasma-integration`, which is the package that makes the variable mean anything. Without it the variable is set and nothing honours it, with no error. That variable used to be `qt6ct`, which is meant for Qt apps that are *not* KDE: with no `~/.config/qt6ct` to read, it served its own default light palette and overrode the dark kdeglobals behind it, which is why Dolphin came up white while a dark scheme was configured. `qt6ct` is no longer installed or declared, since the KDE platform theme covers both kinds. `generate_theme.py` generates the scheme and merges only the `[Colors:*]`, `[WM]` and `[ColorEffects:*]` sections into kdeglobals, leaving KDE's own keys alone. It also sets the font keys, key by key rather than by section, because they sit in `[General]` beside keys that are none of our business. **The weight there is Qt's 0-99 scale, not the CSS one**: writing `500` from `geometry.font_weight_ui` does not fail, it clamps and renders as Black. `57` is the Medium that 500 means. KDE tools also write to that file, so check `git diff` after using a KDE settings dialog.
...
- **The GTK `settings.ini` files are written by KDE, not by hand.** `kded6` is the KDE daemon; it is D-Bus activated, so it starts on any KDE application launch, and it loads a module called `gtkconfig` ("GNOME/GTK Settings Synchronization Service") whose entire job is to copy KDE's font, icon theme, cursor and toolbar settings into both `settings.ini` files. It won every contest with our hand-edited values, reverting the font to Noto Sans and stripping every comment. Disabling it does not work: `[Module-gtkconfig] autoload=false` in `kdedrc` is read (`kreadconfig6` confirms it) and ignored, because the module is also `load-on-demand`. So the source of truth for GTK's font, icons and cursor is **`kdeglobals` and `kcminputrc`**, both generated from `palette.json`'s `typography` block, and `gtkconfig` copies them across for us. Two consequences worth knowing: the module writes only when a value differs, so the symlinks survive once the values agree; and `gtk-theme-name` is not a key it manages, so `adw-gtk3-dark` is ours to keep in `settings.ini` and it is preserved. Comments in those two files never survive.
- **KDE's writes replace symlinks with real files.** `gtkconfig` rewrites `gtk.css` on every start, even when the content is unchanged, and the rename-over-target lands a real file where the symlink was. `backup-configs.sh check` catches it; the repair is `backup-configs.sh install --force`, and it is safe because the content is preserved byte for byte apart from the trailing newline. `kdeglobals` and `kcminputrc` are not affected, since KConfig writes those through the link.
...
- **`swaylock` here is plain swaylock, not `swaylock-effects`, and the config was written for the latter.** Seven options in it were unknown to the installed binary: `screenshots`, `effect-blur`, `effect-vignette`, `indicator`, `clock`, `timestr`, `datestr`. That mattered because `screenshots` was the only thing setting a background, so with no `color=` line the lock screen came up in swaylock's **own default light grey** while all 28 of its colours were correct. Before adding an option here, diff it against `swaylock --help`; the binary ignores what it does not know and says nothing. The background is now solid `void-00`, and there is no clock because plain swaylock has none.
- **`check_palette.py` could not see any swaylock colour until recently.** swaylock's format is `ring-color=393835`, with no leading `#`, and the hex regex requires one, so those 28 values sat unverified from the start. There is now a `BARE_HEX_SCOPE` for that one file, deliberately scoped, because six hex digits with no `#` match a commit hash anywhere else. If another app writes hex without a `#`, add it to that tuple rather than loosening the regex.
- **Clipboard history is `cliphist` over `wl-clipboard`, and the picker is a script.** `scripts/wm/clipboard-picker.sh` holds the whole pipeline because Hyprland and Sway need exactly the same one, and inlining it twice is how the three waybar configs drifted. Two things about it: `wl-paste --watch cliphist store` in both autostarts is the only thing that feeds the history, so without it the keybind opens a permanently empty menu; and the leading id in `cliphist list` output is hidden with wofi's `--pre-display-cmd`, which changes only what is drawn and never what is returned, so a badly quoted entry can draw wrong but can never paste the wrong thing.
- **The idle schedule exists twice and has to be changed twice.** `hypridle` reads `.config/hypr/hypridle.conf`, while `swayidle` has no config file at all and takes its whole schedule as CLI arguments in `.config/sway/config`. Both use the same numbers (lock at 5 min, screen off at 6, suspend at 30) and both hand off to `swaylock` through `loginctl lock-session`, so every path to a locked screen goes through one place.
- **fastfetch colours must be written as hex, never as ANSI codes.** fastfetch accepts both, and `38;2;180;73;85` is the same colour as `#b44955` and is invisible to `check_palette.py`. Ten configs live under `.config/fastfetch/`: `config.jsonc` is what bare `fastfetch` reads, `arch_small/` is the fish greeter, `arch_large/`, `crow/` and `home/` are dormant presets kept aligned so they do not come back wrong, and `minimal/` holds a one-accent variant of each. All ten are validated by `/verify-repo`. Also set `display.percent.type` to `["num"]` on the short presets: the default is `9`, a *coloured* number, which tints by threshold and puts an accent on screen at rest for nothing.
- **Colour in the fetches is assigned by what a row means.** Bordeaux for identity (logo, title, username, hostname), Ice for function (os, kernel, host, cpu, gpu, wm, shell, terminal, fonts, cursor), Moss for duration (uptime, datetime), Ash for media (media, player), ink for counts. The key carries the family and the value stays ink-2, so the content column reads uniformly. Bronze and Verdigris are deliberately absent: rarity is part of Bronze's meaning, and the guide restricts Verdigris to ANSI slots 6 and 14 and to syntax highlighting. Ice means focus on interactive surfaces; a fetch has no focus, which is what frees Ice to mean function there. `.config/catnap/` uses the same mapping, so switching the greeter between them does not change what a colour means.
- **catnap cannot do hex, and its config format is one upstream release from being deleted.** Writing `#b44955` in `config.toml` prints the literal string into the output. The whole vocabulary is seven tokens, and anything unrecognised silently becomes a reset: `(RD)`→31, `(YW)`→33, `(BE)`→34, `(GN)`→32, `(MA)`→35, `(CN)`→36, `(BK)`→30. Those are ANSI slots and our ANSI table is the palette, so catnap cannot drift off-palette; what it cannot reach is a specific level, so its keys sit at ink-2 where fastfetch's sit at ink-4. Separately: **catnap 2.0 replaced TOML with a `.cat` language and upstream says `config.toml` and `distros.toml` are not compatible with v2.** Upstream is 2.1.1, the AUR package is 1.1.1, so both tracked files break outright the day it moves. `/verify-repo` runs catnap and reports its exit code, which is what will catch that.
