...
3. Give the agent a check it can run itself.
...
is final. *A settled decision about an orphaned config read as an open question
and was re-raised three sessions running.*

### Difficulty and priority are independent **(paid)**

Rate both, separately. A one line fix can be urgent and a rewrite optional.
*Collapsed into one axis, two trivial broken keybinds sat behind a large
optional refactor.*
...
work" is a full restart. *A waybar bug survived a full bisect; only the written
eliminations stop the next session repeating it.*
...
surrounding file, never two languages in one. *279 em dashes had to be cleaned
out of two design documents once the tic was noticed.*
...
is the proof it can be, and reserve prose for judgement calls. *A documented
rule to mirror theming across two window managers went unenforced through an
entire retheme; a twenty line checker found it in one run.*
...
you are the verification loop. *Three things here were configured, looked
configured, and did nothing, with no error message in any of the three.*
...
it. *Applies to any project with codegen; skip it if nothing is generated.*
...
you are doing. *An override that matched half its keys produced a themed surface
under a stock accent colour, which read as a bug in our code.*
...
*Bulk rewrites in this repo are routinely done through shell heredocs, which are
exactly the case checkpoints miss.*

### Investigation goes to a subagent **(borrowed)**
...
*A session that starts unnamed drifts; one that ends without updating the list
leaves the next to reconstruct where things stood from a diff.*
